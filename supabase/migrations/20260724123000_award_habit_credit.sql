-- Unificación PTS → Créditos: hábitos personales con tope diario anti-abuso.
-- action_key = HABITO:{reto_id}, period_key = day:YYYY-MM-DD
-- Tope acumulado por día entre todos los hábitos (default 15).
...
create or replace function public.award_habit_credit(
  ...
  p_daily_cap integer default 15,
...
begin
  ...
  daily_cap := greatest(coalesce(p_daily_cap, 15), 0);
...
comment on function public.award_habit_credit(uuid, text, integer, text, integer, jsonb) is
  'Otorga créditos por hábito personal con tope diario acumulado (anti-abuso). No afecta rachas.';

grant execute on function public.award_habit_credit(uuid, text, integer, text, integer, jsonb)
  to authenticated, service_role;

comment on column public.profiles.credits is
  'Saldo de créditos (única moneda). Core (escaneo/plan/share/referidos) + hábitos con tope diario.';
