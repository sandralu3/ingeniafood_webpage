...
export type PremiumAccess = {
  ...
  /** Generaciones de foto OpenAI restantes (legacy testers). */
...
export function resolvePremiumAccess(
  ...
  if (!isTester) {
    // Soft-launch: sin UI Premium/Stripe para no testers.
    return EMPTY_PREMIUM_ACCESS;
  }

  const isPaidPremium = Boolean(profile?.is_premium);
  ...
  return {
    ...
    premiumTrialRemaining: 0,
    premiumTrialClaimed,
    canUsePremiumFeatures: isPaidPremium,
    canSimulatePremiumTrial: false
  };
}
